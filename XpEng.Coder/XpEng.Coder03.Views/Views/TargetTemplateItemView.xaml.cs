...
namespace XpEng.Coder03.Views.Views {
    public partial class TargetTemplateItemView : UserControl {
        ...
        private void BrowseTargetDirectory_Click(object sender, RoutedEventArgs e) {
            var dialog = new OpenFolderDialog { Title = "Select Target Output Directory" };
            var vm = DataContext as dynamic;

            if (vm != null && !string.IsNullOrWhiteSpace(vm.TargetDirectory) && Directory.Exists(vm.TargetDirectory)) {
                dialog.InitialDirectory = vm.TargetDirectory;
            }
        ...
        private void BrowseTemplate_Click(object sender, RoutedEventArgs e) {
            ...
            if (vm != null && !string.IsNullOrWhiteSpace(vm.TemplatePath)) {
                string? directory = Path.GetDirectoryName((string)vm.TemplatePath);

                if (!string.IsNullOrWhiteSpace(directory) && Directory.Exists(directory)) {
                    dialog.InitialDirectory = directory;
                    dialog.FileName = Path.GetFileName((string)vm.TemplatePath);
                }
            }
