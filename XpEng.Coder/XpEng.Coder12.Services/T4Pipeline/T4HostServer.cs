...
public sealed class T4HostServer : IDisposable {
    ...
    private async Task AcceptLoopAsync(CancellationToken ct) {
        while (!ct.IsCancellationRequested) {
            ...
            try { await server.WaitForConnectionAsync(ct).ConfigureAwait(false); }
    ...
    public void Dispose() {
        _lifetimeCts.Cancel();
        _pendingServer?.Dispose(); // forces the currently-pending WaitForConnectionAsync to unblock — cancellation tokens don't reliably interrupt this on their own
        _lifetimeCts.Dispose();
        Status = T4HostStatus.Disposed;
        Logger.Log("[T4Host] Disposed.");
    }
    #endregion IDispose
}